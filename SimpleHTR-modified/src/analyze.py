...
class Constants:
	...
	vanilla_fnTranslationInvariance = '../data/vanilla_translationInvariance.npy'
...
def showResults():
	...
	vanilla_probs = np.load(Constants.vanilla_fnTranslationInvariance)
	f = open(Constants.fnTranslationInvarianceTexts, 'rb')
	texts = pickle.load(f)
	texts = ['%d:'%i + texts[i] for i in range(len(texts))]
	f.close()
	
	plt.figure('Translation invariance', figsize=(8,5))
	plt.rcParams.update({'font.size': 16})
	plt.plot(probs, 'o-')
	plt.plot(vanilla_probs, 'ro-')
	plt.xticks(np.arange(len(texts), step=5), rotation='horizontal')
	plt.xlabel('horizontal translation (pixels)')
	plt.ylabel('text probability of given image')
	plt.ylim(0, 1)
